  ...
  Future<void> updateProfile(BuildContext context) async {
    ...
    try {
      ...
      final response = await http.put(
        Uri.parse('${Environment.backendUrl}/api/auth/profile'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      if (response.statusCode == 200) {
        ...
        showToast(context, _successMessage, isSuccess: true); // ✅ Top toast
      } else {
        final errorData = jsonDecode(response.body);
        _errorMessage = errorData['message'] ?? 'Failed to update profile.';
        showToast(context, _errorMessage, isSuccess: false); // ✅ Top toast
      }
    } catch (e) {
      _errorMessage = 'An error occurred: $e';
      showToast(context, _errorMessage, isSuccess: false); // ✅ Top toast
